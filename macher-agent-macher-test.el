...
(describe "Macher-Agent Macher bridge suite"
  ...
  (describe "macher-agent-macher-build-patch and ephemeral translation"
    (it "passes canonical relative paths to macher--make-context"
      (unless (fboundp 'macher--make-context)
        (defalias 'macher--make-context (lambda (&rest _) nil)))
      (let* ((proj-root "/mock/canonical-project")
             ...
             (macher-make-context-calls nil))
        (cl-letf (((symbol-function 'macher--make-context)
                   (lambda (&rest args)
                     (setq macher-make-context-calls (append macher-make-context-calls (list args)))
                     (list :mock-context t)))
                  ((symbol-function 'macher--build-patch)
                   (lambda (_c _f) 'patch-ok)))
          (let ((res (macher-agent-macher-build-patch agent-ctx "Canonical test" files)))
            (expect res :to-equal 'patch-ok)
            (expect (length macher-make-context-calls) :to-equal 1)
            (let ((mc-args (car macher-make-context-calls)))
              ...
              (expect (plist-get mc-args :prompt) :to-equal "Canonical test")
              (expect (plist-get mc-args :dirty-p) :to-be t)))))))
  ...
  (describe "Context lookup and resolution pipeline without obsolete steps"
    ...
    (it "installs clean context-resolution pipeline steps and excludes obsolete steps"
      ...
      (let ((steps (macher-agent-get-pipeline-steps 'context-resolution)))
        ...
        (expect (member 'macher-agent-resolve-from-transit-payload steps) :to-be nil)
